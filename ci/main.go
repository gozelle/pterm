	...
	do("Running PTerm CI System", 1, func(currentLevel int) {
		...
		do("Geneating Printers Table", currentLevel, func(currentLevel int) {
			...
			for i, feature := range allPrinters {
				...
				// Format: "[Example](https://github.com/gozelle/pterm/tree/master/_examples/FEATURE)"
				if i%5 == 0 {
					tableContent += "| "
				}
				name := strings.ToUpper(string(feature[0])) + feature[1:]
				name = strings.ReplaceAll(name, "_", " ")
				tableContent += fmt.Sprintf("%s <br/> [(Examples)](https://github.com/gozelle/pterm/tree/master/_examples/%s) |", name, feature)
				if (i+1)%5 == 0 {
					tableContent += "\n"
				}
